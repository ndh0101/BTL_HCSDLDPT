...
def build_clusters(k=20):
    """
    Đọc vector từ CSDL, chạy thuật toán K-Means gom cụm 
    và tạo Chỉ mục ngược (Inverted File Index - IVF).
    """
    # Đọc dữ liệu vector đã trích xuất
    if not os.path.exists(FEATURE_FILE):
        print(f"Không tìm thấy file {FEATURE_FILE}. Hãy chạy indexer.py trước.")
        return
    with open(FEATURE_FILE, 'rb') as f:
        features_dict = pickle.load(f)
    if not features_dict:
        print("Dữ liệu vector trống.")
        return
    # Chuẩn bị ma trận dữ liệu cho thuật toán K-Means
    list_ids = list(features_dict.keys())
    list_vectors = list(features_dict.values())
    # Chuyển đổi thành ma trận Numpy (N x D)
    # Trong đó N là số ảnh, D là số chiều vector
    ...
    print("\nThống kê phân bố ảnh trong các cụm:")
